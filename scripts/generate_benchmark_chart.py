...
def load_raw_ttft(proxy_path: Path, concurrency: int = 1) -> list[float]:
    ...
    return [
        r["ttft_ms"] for r in raw if r["concurrency"] == concurrency and r["ttft_ms"] is not None
    ]
...
def chart_distribution(ttft_ms: list[float]) -> None:
    ...
    ax.axvline(
        np.percentile(ttft_ms, 50),
        color="#1a1d24",
        linestyle="--",
        linewidth=1.2,
        label=f"P50 {np.percentile(ttft_ms, 50):.0f} ms",
    )
    ax.axvline(
        np.percentile(ttft_ms, 99),
        color="#e5484d",
        linestyle="--",
        linewidth=1.2,
        label=f"P99 {np.percentile(ttft_ms, 99):.0f} ms",
    )
    ax.set_xlabel("TTFT (ms)")
    ax.set_ylabel("Requests")
    ax.set_title(
        f"Proxy TTFT distribution · concurrency 1 · n={len(ttft_ms)}",
        fontsize=10,
        fontweight="bold",
    )
    ax.spines[["top", "right"]].set_visible(False)
...
def main() -> None:
    ...
    for name in (
        ...
        "benchmark-ttft-percentiles.png",
        "benchmark-ttft-distribution.png",
    ):
